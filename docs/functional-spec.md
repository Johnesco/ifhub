...
- Cards fill a responsive grid (columns of at least 320px), sorted by title. A card shows the title (linking to the game's landing page), badges for the engine and, when any version has them, sound and tests, the author line, the description, and for a versioned group a Version pulldown: the primary first, then the other versions newest first. One pulldown keeps every card the same shape however many versions a group has; as a row of chips, Family Zoo's seventeen took most of its card.
...
Fifteen themes live in `themes.js`: classic (default), dos, apple2, c64, amiga, mac, atarist, cpm, atari8, trs80, sepia, midnight, forest, lavender, solarized. Each defines `chrome` (hub UI colors and font), `game` (colors, fonts and sizes pushed into game pages), `syntax` (the source pane's eight highlight colors) and `scrollbar`. The choice persists in `localStorage` under `ifhub-theme`; pages apply the chrome by setting CSS custom properties on the root element. Fonts load on demand from two places: Google Fonts for the hub chrome faces, and the hub's own `fonts.css` for the machine character sets it self-hosts.

Classic is an ordinary theme: it is the default, and it is applied like any other rather than meaning "leave everything alone". Every theme reaches every surface — hub chrome, the source pane, the walkthrough viewer, the game and the tests report — and all of them follow the same selection, so a shared `?theme=` link looks the same for the reader as for the sender.

Every text pairing a theme draws (page, cards, toolbar, buttons, chips, badges, code, and the game panes' text, input, emphasis, headers and status line) meets WCAG AA contrast of 4.5:1, with one deliberate exception: Solarized's muted text on its card background stays at Solarized's own 4.1:1. The `syntax` block carries its own values rather than deriving them from the chrome, because the narrow palettes cannot supply eight hues: a phosphor terminal or a black-on-white Mac separates the classes by brightness, the way those machines did, and Apple II, CP/M, TRS-80, Macintosh and Atari ST are written that way. Every syntax color clears 4.5:1 against its own theme's `codeBg`, except comments, which are meant to recede and are held at 3:1 while also being italic, and Solarized, which keeps its own ratios as it already does elsewhere.

A theme named after a system puts that machine's own character set in the game pane, because the game is where the platform illusion has to land. MS-DOS renders in IBM VGA 8×16, Apple II in the 40-column character ROM, Atari 800 in ATASCII, Atari ST in the TOS system font, TRS-80 in the Model III ROM, the C64 in its own character ROM and the Amiga in the AmigaOS 1.2/1.3 Topaz, and CP/M keeps VT323 because Kaypro terminals emulated the ADM-3A. Google Fonts carries none of the first seven, so `site/fonts.css` declares them against files in `site/fonts/` and `ensureRetroFonts()` injects that stylesheet into the hub and into every game iframe — a browser downloads only the face the selected theme names. Each face is a bitmap traced to outlines, so each theme's `bufferSize` is that face's native pixel height or a whole multiple of it (16px for IBM VGA 8×16, Print Char 21, Candy Antics, Project Jason Tall, Pet Me 64 and Topaz; 24px for the Model III face); moving off those values is what makes a pane blurry. No font carries a CRT effect: a scanline look distorts the glyphs, so it belongs on the screen, where it can be tuned and switched off, and never in the outlines, where it only resolves at a few sizes and goes lumpy at the rest. Topaz is built from the Amiga disk font by `tools/build_topaz.py`, which traces every pixel, so the file can be regenerated rather than edited. The source, walkthrough and tests panes keep their readable fonts and follow the theme in color only. A BASIC game draws its own text into a canvas, which no stylesheet can reach, so it keeps the emulator's font whatever the theme.

A theme named after a system uses that system's palette: MS-DOS is CGA light gray on black with the Norton blue for fills, Commodore 64 draws every value from the C64 palette exactly — white on the machine's blue in the game pane, black behind the source with yellow, light green and cyan on it — and Amiga anchors on Workbench 1.3's four colors, white text on Workbench blue with orange kept for the chrome, and the secondary shades held to tints of those same two hues, Atari 800 is GRAPHICS 0 — the stock `$94` blue with its text one luminance step above stock, because GRAPHICS 0 can only vary luminance within the background's hue and the stock `$9A` is 3.74:1, with its chrome drawn from the same hue a step darker so the screen reads as the lit thing in its frame, every value a GTIA palette entry —  TRS-80 is the Model III's white-gray phosphor on black, CP/M (Kaypro) is green phosphor on black, Apple II is green phosphor, Macintosh and Atari ST are black on white. Where a theme names a machine Infocom shipped on, its status line is drawn in reverse video, because that is what every Infocom interpreter did: the Amiga's is a white bar with blue text, the Atari ST's a black bar with white, the MS-DOS and TRS-80 ones light bars with dark text.

### 5.2 Style dropdown and overlays

In the player the Style dropdown lists the platform themes, then a separator, then **Native**. For a game with `overlayLabel` its own overlay comes first, before the themes, and is that game's default.
...
|---|---|---|---|
| `ifhub:applyTheme` | hub → game | `game`, `scrollbar` | apply the theme colors; the game adds `body.platform-theme-active` and hides its own effects |
...
The hub does not build or test games. Each engine workspace has a `tools/build.py`; the hub's `tools/ship.py` verifies the folder, writes the landing page, sets `hub = yes`, publishes the game repo, and only then regenerates the data files and pushes the hub — so a failed publish leaves the hub unchanged rather than listing a game that 404s. `tools/check_links.py --deployed` audits the registry against the live site, and `tools/check_drift.py` reports which games carry a stale copy of the Pages workflow or landing page the hub wrote into them (`--fix` re-publishes them; `--deployed` reads each remote's default branch instead of the folder, which is what catches a publish that landed somewhere Pages does not deploy from). See `docs/publishing.md`.
