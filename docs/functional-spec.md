...
A theme named after a system uses that system's palette: MS-DOS is CGA light gray on black with the Norton blue for fills, Commodore 64 draws every value from the C64 palette exactly — white on the machine's blue in the game pane, black behind the source with yellow, light green and cyan on it — and Amiga anchors on Workbench 1.3's four colors, white text and an orange status line on Workbench blue, with the secondary shades kept to tints of those same two hues, Atari 800 is GRAPHICS 0 light blue on blue, TRS-80 is the Model III's white-gray phosphor on black, CP/M (Kaypro) is green phosphor on black, Apple II is green phosphor, Macintosh and Atari ST are black on white.
