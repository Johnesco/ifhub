...
A theme named after a system puts that machine's own character set in the game pane, because the game is where the platform illusion has to land. MS-DOS renders in IBM VGA 8×16, Apple II in the 40-column character ROM, Atari 800 in ATASCII, Atari ST in the TOS system font and TRS-80 in the Model III ROM; the C64 uses Sixtyfour and the Amiga uses Workbench, both derived from those machines, and CP/M keeps VT323 because Kaypro terminals emulated the ADM-3A. Google Fonts carries none of the first five, so `site/fonts.css` declares them against files in `site/fonts/` and `ensureRetroFonts()` injects that stylesheet into the hub and into every game iframe — a browser downloads only the face the selected theme names. Each face is a bitmap traced to outlines, so each theme's `bufferSize` is that face's native pixel height or a whole multiple of it (16px for IBM VGA 8×16, Print Char 21, Candy Antics and Project Jason Tall; 24px for the Model III face); moving off those values is what makes a pane blurry. Workbench is sized for the same reason from the other direction: it is a variable font whose `SCAN` and `BLED` axes draw CRT scanline detail, and below about 20px that detail falls under a pixel and antialiases into gray fuzz, so the Amiga pane renders at 24px. The source, walkthrough and tests panes keep their readable fonts and follow the theme in color only. A BASIC game draws its own text into a canvas, which no stylesheet can reach, so it keeps the emulator's font whatever the theme.
