...
4. **Publish** — `tools/publish.py` commits everything in the game folder and pushes to `<org>/<game>`; on a first publish it creates the repo, adds the Pages workflow, and enables Pages. Every git and gh step is checked: if any fails the script exits non-zero and shipping stops here, with the hub registry untouched. The org is `IFHUB_GH_ORG` (default `Johnesco`); the commits carry `Co-Authored-By: $IFHUB_COAUTHOR` when that is set and no trailer otherwise, so set it when Claude is driving and leave it unset when you ship by hand. It refuses to publish from any branch but the one the remote deploys from (a clean checkout left on a feature branch would otherwise commit, push, deploy nothing, and report success), and it finishes a push that an earlier run committed but failed to send. It also refuses when `origin` points at any repo other than `<org>/<game>`, so a mis-set remote cannot push a game into some other repository.
