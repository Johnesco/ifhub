...
IF Hub is **receive-only**. It displays a game, its source, and its walkthrough (plus a test report and a stylesheet when the game ships them). It does not build, compile, or test games. Every engine has its own workspace under `C:\code\text-games\<engine>\` with a `tools/build.py`; the hub takes the folder that produces and puts it online.
...
|---|---|---|
| `ifhub.conf` | yes | `engine`, `title`, `description`, `tags`, `source`, `style`, `walkthrough`, `sound`, and `hub = yes` to be listed |
...
| stylesheet | no | the file named by `style =` (a mood overlay, an author stylesheet); the hub shows it in the CSS tab, highlighted like source |
...
- `site/app.html` is the split-pane player: game, source, walkthrough, tests, and CSS panes. Source is fetched raw and highlighted in the hub (Inform 7, Rez, Ink, BASIC, Chord; games with `sourceBrowser = yes` are iframed instead); the CSS pane shows the stylesheet named by `style =` through the same code table; walkthroughs render in `site/walkthrough.html?game=<id>` from the game's txt files; 15 platform themes from `themes.js`; collections from `hubs.json` (filter by engine or tag, switched client-side). Theming reaches into game pages through `theme-listener.js` (each workspace ships a copy) and `ifhub:applyTheme` messages. See `reference/css-overlay.md`.
...
| `text-games/sharpee/` | `tools/build.py <game>` | Sharpee (Chord): `sharpee compose --check`, `build`, `test`; play.html from the built page with the theme listener, the `.story` highlighted by the hub, tests.html, walkthrough files |

Each workspace has its own `CLAUDE.md` with authoring rules and is a git repo that holds only the tooling (GitHub: `Johnesco/inform7-workspace`, `ink-workspace`, `rez-workspace`, `basic-workspace`, `sharpee-workspace`); the game folders inside it are ignored because every game is its own repo.
